...
export class Tree {
	...
	/**
			...
			* @return {DIVElement}
			*/
	insertItem(data, parent_id, position, options) {
		if (!parent_id) {
	...
	createAndInsert(data, options, parent_id, element_index) {
		//find parent
	...
	_insertInside(element, parent_index, offset_index, level) {
	...
	_findElementLastChildIndex(start_index) {
		if (start_index == -1)
			return -1;
	...
	_findChildElements(id, only_direct) {
	...
	/**
			...
			* @param {string} name
			*/
	filterByRule(callback_to_filter, param) {
	...
	/**
			...
			* @param {string} id
			*/
	setSelectedItem(id, scroll, send_event) {
		if (!id) {
			//clear selection
	...
	/**
			...
			* @return {Array}
			*/
	getAncestors(id_or_node, result) {
		result = result || [];
		var element = this.getItem(id_or_node);
		if (element) {
			result.push(element);
	...
	/**
			...
			* @return {bool}
			*/
	removeItem(id_or_node, remove_children) {
		var node = id_or_node;
	...
	markAsSelected(node, add_to_existing_selection) {
		//already selected
		if (node.classList.contains("selected"))
			return;
	...
	_updateListBox(node, options, current_level) {
		if (!node)
			return;
